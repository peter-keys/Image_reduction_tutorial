...
;
function reg, scene, ref, kernel, $
              apply_scene=apply_scene, $
              disp=disp_out, rdisp=rdisp	; register scene(s) with respect
...
rdisp_sz = SIZE(rdisp)
disp_out = FLTARR(rdisp_sz(1),rdisp_sz(2),rdisp_sz(3),nf)
; assures that all four dimensions are maintained even if nf = 1
disp_out = REFORM(disp_out,rdisp_sz(1),rdisp_sz(2),rdisp_sz(3),nf)
...
;    print, 'rms =', rms
    ...
    ;disp_diff = disp - rdisp
    ;disp      = rdisp + disp_diff * 0.6
