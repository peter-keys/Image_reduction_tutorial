FUNCTION ROSA_destretch_noRAM4, image_seq, kernels=kernels, disp_mask=disp_mask, destr_vect_raw=destr_vect_raw, redpath=redpath

;+
; NAME:
;       ROSA_destretch_noRAM4
; PURPOSE:
...
;	Peter Keys,	November, 2016 
;
;	NOV 2016 Additions: Cleaned up notes etc for other users
;			    Added a directory keyword
;			    This keyword allows data to be saved 
;			    in a final directory instead of having 
;			    to run in the same directory. 
;			    Therefore allows the user to process 
;			    everything from the raw directory and 
;			    makes processing a little more streamlined				 
;
;-
;--------------------------------------------------------------------------
...
;smooth_num = 5
;smooth_num = 21 	;<------ good seeing with AR
smooth_num = 51		;<------ decent for the filament/QS
median_num  = 5
;smooth_num = 100 	;<------ poor seeing
...
IF file_list THEN BEGIN
   FOR i=0,(numims-1) DO BEGIN
       ...
       WRITEFITS,redpath+'destretched_'+filename,new_image
   ENDFOR
ENDIF
