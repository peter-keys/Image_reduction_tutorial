function SXPAR, hdr, name, abort, COUNT=matches, COMMENT = comments, $
                                  ...
                                  accept_long_string=accept_long_string
;+
; NAME:
;      SXPAR
; PURPOSE:
...
;----------------------------------------------------------------------
 ...
 if abort_return then On_error,0 else On_error,0
...
;  value of the number field in NUMBER.
        ...
        histnam = (nam eq 'HISTORY ') or (nam eq 'COMMENT ') or (nam eq 'HIERARCH') or (nam eq '') 
  ...
  if histnam then $
      ...
      if ( strmid(svalue[i],0,1) EQ "'" ) then begin   ;Is it a string?
          ...
          NEXT_APOST:
                  ...
                  if endap LT 0 then begin
                      IF Keyword_Set(accept_long_string) THEN endap = strlen( svalue[i] ) $
                      ELSE MESSAGE,'Value of '+name+' invalid in '+abort
                  endif
